...
df = pd.read_csv(
    os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "data", "pisa_spain_sample_v2.csv"
    )
)
...
# Defina o caminho absoluto para o diretório img
img_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "img")

# Substitua todas as ocorrências de os.path.join("img", ...) por os.path.join(img_dir, ...)
# Por exemplo:
...
plot_imputation_distribution_assessment(
    df.loc[df_imp.index],
    df_imp,
    ["COMPICT", "BODYIMA", "PERCOOP", "SOCONPA"],
...
dr = DimensionalityReduction(
    df_, num_vars=num_vars, cat_vars=cat_vars, num_algorithm="spca"
)
...
dr.plot_num_main_contributors(
    dim_idx=3, output_path=os.path.join("img", "dim_red_main_contributors.jpg")
)

# Explain component extracted from categorical variables
print(dr.cat_main_contributors_stats())
dr.plot_cat_main_contributor_distribution(
    dim_idx=0, output_path=os.path.join("img", "dim_red_cat_component.jpg")
)

# Explained variance + elbow method
dr.plot_num_explained_variance(
    0.5,
    plots=["cumulative", "normalized"],
...
cl = Clustering(
    df_t,
    algorithms=[KMeans(random_state=42), AgglomerativeClustering(linkage="ward")],
    normalize=False,
)
...
cl.plot_optimal_components_normalized(
    output_path=os.path.join("img", "clustering_elbow_curve.jpg")
)
...
cl.plot_distribution_comparison_by_cluster(
    df_ext=df_[["ESCS", "TEACHSUP"]],
    output_path=os.path.join("img", "clustering_distribution_comparison.jpg"),
)

# 2-D plots
cl.plot_clusters_2D(
    "dim_01", "dim_02", output_path=os.path.join("img", "clustering_2d_plots.jpg")
)
...
cl.plot_cat_distribution_by_cluster(
    df_["IMMIG"],
    cat_label="IMMIG",
    cluster_label="Student clusters",
...
classifier.plot_shap_importances(
    output_path=os.path.join("img", "classifier_global_feature_importance.jpg")
)
plt.clf()

# Plot local importance for clusters with identifiers 1 and 2
classifier.plot_shap_importances_beeswarm(
    class_id=1, output_path=os.path.join("img", "classifier_local_importance_cl1.jpg")
)
plt.clf()
classifier.plot_shap_importances_beeswarm(
    class_id=2, output_path=os.path.join("img", "classifier_local_importance_cl2.jpg")
)
plt.clf()
...
classifier.plot_confusion_matrix(
    output_path=os.path.join("img", "classifier_confusion_matrix.jpg")
)
...
# Build a pipeline with feature selection, hyperparameter tuning, and model fitting. For feature selection, we make sure
# that variable ESCS (economic, social and cultural status) gets selected (features_to_keep=['ESCS']). Hyperparameter
# optimization is performed through exhaustive
