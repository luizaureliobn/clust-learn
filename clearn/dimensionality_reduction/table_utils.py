...
def cross_corr(df1, df2):
    ...
    corr_df = pd.DataFrame(
        corr_coeffs, index=df1.columns, columns=df2.columns
    ).transpose()
    return corr_df
...
def num_main_contributors(
    ...
    for idx, row in corrs.iterrows():
        pc_corrs = (
            row[(row < -thres) | (row > thres)]
            .to_frame()
            .reset_index()
            .rename(columns={"index": "var_name", idx: "corr_coeff"})
        )
        pc_corrs["component"] = idx
        pc_corrs["corr_coeff_abs"] = np.abs(pc_corrs["corr_coeff"])
        pc_corrs = pc_corrs.sort_values("corr_coeff_abs", ascending=False).drop(
            columns="corr_coeff_abs"
        )
    ...
    if component_description is not None:
        ...
        component_description = pd.DataFrame(
            data={
                "component": mc["component"].unique(),
                "component_description": component_description,
            }
        )
    ...
    if col_description is not None:
        if not isinstance(col_description, pd.DataFrame):
            raise RuntimeError(
                "`col_description` must be an instance of pandas.DataFrame"
            )
...
def cat_main_contributors(
    ...
    for idx, row in corrs.iterrows():
        pc_corrs = (
            ...
            .to_frame()
            .reset_index()
            .rename(columns={"index": "var_name", idx: "corr_ratio"})
        )
        pc_corrs["component"] = idx
        pc_corrs = pc_corrs.sort_values("corr_ratio", ascending=False).reset_index(
            drop=True
        )
...
def cat_main_contributors_stats(
    df, df_trans, thres=0.14, n_contributors=None, dim_idx=None, output_path=None
):
    """
    ...
    m2 = pd.melt(
        df_trans.reset_index().rename(columns={"index": "id"}),
        id_vars=["id"],
        value_vars=df_trans.columns,
    )
    stats = m1.merge(m2, on="id")
    stats = (
        stats.groupby(["variable_y", "variable_x", "value_x"])
        .agg({"value_y": ["mean", "std"]})
        .reset_index()
        .rename(
            columns={
                "variable_y": "component",
                "variable_x": "var_name",
                "value_x": "var_value",
            }
        )
