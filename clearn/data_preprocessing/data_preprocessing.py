...
def imputation_pairs(
    ...
    final_pairs = (
        final_pairs.merge(missing_df, left_on="var1", right_on="var_name")
        .rename(columns={"missing": "missing_var1"})
        .drop(columns="var_name")
    )
    final_pairs = (
        final_pairs.merge(missing_df, left_on="var2", right_on="var_name")
        .rename(columns={"missing": "missing_var2"})
        .drop(columns="var_name")
    )
    ...
    final_pairs = final_pairs.loc[
        final_pairs.groupby("var1")["missing_var2"].idxmin()
    ].sort_values(["missing_var2", "missing_var1"])
    final_pairs = final_pairs[
        final_pairs["missing_var1"] > final_pairs["missing_var2"]
...
def impute_missing_values_with_highly_related_pairs(
    df_original, imputation_pairs, num_vars=None, cat_vars=None
):
    """
    ...
    * For mixed-type variables, the numerical one is discretized by bucketization into quantiles and both variables are
    ...
    for idx, row in imputation_pairs.iterrows():
        if row["var1"] in num_vars and row["var2"] in num_vars:
            ...
            df_f = df[(~df[row["var2"]].isnull()) & (df[row["var1"]].isnull())]
            df.loc[df_f.index, row["var1"]] = lr.predict(
                df_f[row["var2"]].values.reshape(-1, 1)
            )

        else:
            print(
                f"Imputing with empirical discrete distribution {row['var2']} -> {row['var1']}"
            )
            ...
            for pv in predictor_values:
                ...
                dist_freq = (
                    df.loc[
                        (df[f"b_{row['var2']}"] == pv) & (~df[row["var1"]].isnull()),
                        row["var1"],
                    ]
                ...
                if dist_freq.shape[0] > 0:
                    ...
                    df.loc[
                        (df[f"b_{row['var2']}"] == pv) & (df[row["var1"]].isnull()),
                        row["var1"],
                    ] = disc_dist.ppf(np.random.rand(no_missing))
...
def mutual_information_pair_scores(df, num_vars=None, cat_vars=None):
    ...
    for nv in num_vars:
        ...
        mi = mutual_info_regression(
            values.reshape(-1, 1), values, discrete_features=False, random_state=42
        )[0]
        max_mi_scores[nv] = mi
    for cv in cat_vars:
        ...
        for j in range(i + 1, len(num_vars)):
            ...
            mi = max(
                mutual_info_regression(
                    df_f[num_vars[i]].values.reshape(-1, 1),
                    df_f[num_vars[j]],
                    discrete_features=False,
                    random_state=42,
                )[0],
                mutual_info_regression(
                    df_f[num_vars[j]].values.reshape(-1, 1),
                    df_f[num_vars[i]],
                    discrete_features=False,
                    random_state=42,
                )[0],
    ...
    for i in range(len(num_vars)):
        for j in range(len(cat_vars)):
            ...
            mi = max(
                ...
                mutual_info_regression(
                    df_f[cat_vars[j]].values.reshape(-1, 1),
                    df_f[num_vars[i]],
...
def variable_graph_partitioning(pair_scores, thres=0.05):
    ...
    """
    edges = (
        pair_scores[pair_scores["score"] > thres]
        .apply(lambda row: (row["var1"], row["var2"]), axis=1)
        .tolist()
    )
    ...
    all_nodes = list(
        np.unique(pair_scores["var1"].to_list() + pair_scores["var2"].to_list())
    )
...
def hot_deck_imputation(df_original, variables, k=8, partitions=None):
    ...
    for p in partitions:
        ...
        df_p_i = pd.DataFrame(
            data=imputer.fit_transform(df_p[p]), index=df_p.index, columns=p
        )
...
def impute_missing_values(
    ...
    ip = imputation_pairs(
        ...
        num_vars,
        cat_vars,
        num_kws=num_pair_kws,
        mixed_kws=mixed_pair_kws,
        cat_kws=cat_pair_kws,
    )
    ...
    print(
        f"--- Removing records with more than {max_missing_thres * 100} missing values ---"
    )
...
def remove_outliers(df, variables, iforest_kws=None):
    ...
    if iforest_kws is None:
        iforest_kws = dict(
            max_samples=0.8, max_features=0.8, bootstrap=False, random_state=42
        )
