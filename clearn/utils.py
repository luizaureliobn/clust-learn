...
def compute_high_corr_pairs(df, corr_thres=0.7, method="pearson"):
    ...
    *Note* all variables must be numerical.
    ...
    num_pairs = (
        corr_df.replace({1: 0})[np.abs(corr_df.replace({1: 0})) > corr_thres]
        .reset_index()
        .melt(id_vars="index", value_vars=corr_df.columns[1:])
        .dropna()
...
def compute_highly_related_mixed_vars(df, num_vars, cat_vars, np2_thres=0.14):
    ...
    pairs = (
        cross_corr_df[cross_corr_df > np2_thres]
        .reset_index()
        .melt(id_vars="index", value_vars=cross_corr_df.columns[1:])
        .dropna()
        .sort_values("value", ascending=False)
        .rename(columns={"index": "var1", "variable": "var2"})
        .reset_index(drop=True)
    ...
    pairs = pd.concat(
        [pairs, pairs.rename(columns={"var1": "var2", "var2": "var1"})],
        ignore_index=True,
    )
    pairs = pairs.sort_values(
        ["value", "var1", "var2"], ascending=[False, True, True]
    ).reset_index(drop=True)
    return pairs
...
def compute_highly_related_categorical_vars(df, mi_thres=0.6):
    ...
    cat_pairs = (
        pd.DataFrame(data=data, columns=["var1", "var2", "value"])
        .sort_values(["value", "var1", "var2"], ascending=[False, True, True])
        .reset_index(drop=True)
    )
    return cat_pairs
...
def cross_corr_ratio(df1, df2):
    ...
    for col in df1.columns:
        ...
        for dv in df2.columns:
            col_corr_coeffs.append(
                pg.anova(data=df_aux, dv=dv, between=col)["np2"].iloc[0]
            )
...
def plot_optimal_normalized_elbow(
    values, kl, ax, optimal_label="", xlabel="", ylabel=""
):
    ...
    ax.axvline(
        kl.knee, linestyle="--", linewidth=1, color="#E73F74", label=optimal_label
    )
